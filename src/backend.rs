...
impl CodeGenerator {
    ...
    fn emit_operation(&mut self, op: &lexer::Op, ty: &ast::Type) {
        match op {
            lexer::Op::Add => self.asm.push_str("    add x8, x8, x9\n"),
            lexer::Op::Sub => self.asm.push_str("    sub x8, x8, x9\n"),
            lexer::Op::Mul => self.asm.push_str("    mul x8, x8, x9\n"),
            ...
            // lexer::Op::BwAnd => self.asm.push_str("    add x8, x8, x9\n"),
            // lexer::Op::BwOr => self.asm.push_str("    add x8, x8, x9\n"),
            // lexer::Op::BwXor => self.asm.push_str("    add x8, x8, x9\n"),
            // lexer::Op::BwNot => self.asm.push_str("    add x8, x8, x9\n"),
            // lexer::Op::Mod => self.asm.push_str("    add x8, x8, x9\n"),
            // lexer::Op::Lt => self.asm.push_str("    add x8, x8, x9\n"),
            // lexer::Op::Gt => self.asm.push_str("    add x8, x8, x9\n"),
            // lexer::Op::Lte => self.asm.push_str("    add x8, x8, x9\n"),
            // lexer::Op::Gte => self.asm.push_str("    add x8, x8, x9\n"),
            // lexer::Op::Eq => self.asm.push_str("    add x8, x8, x9\n"),
            // lexer::Op::Neq => self.asm.push_str("    add x8, x8, x9\n"),
            // lexer::Op::Pwr => self.asm.push_str("    add x8, x8, x9\n"),
                ...
                KlirNode::Expr(expr) => {
                    match &expr.lhs {
                        ArgType::Sym(name) | ArgType::Temp(name) => {
                            ...
                            self.emit_typed_load(&ty, self.stackptr, sym_addr);
                        }
                        ArgType::Imm(val) => {
